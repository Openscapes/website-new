---
title: ""
# title-block-banner: images/banner_imgs/horst_openscapes_arctic_1500px.png
---

::: column-screen
![](images/banner_imgs/text_banners/who_are_we_banner.png)

<br>

::: white-div
::: {.medium-body-text .center-text}
**Openscapes is an approach for doing better science for future us.**
:::

::: center-text
We offer mentorship, teaching, coaching and community organizing centered around open data science: we help teams develop collaborative practices that are more reproducible, transparent, inclusive, and kind.
:::

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "Two purple squid, swimming side-by-side heading left."
knitr::include_graphics("images/minis/horst_openscapes_arctic_squid.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_squid.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR STORY

The Openscapes story starts with file names like "data_analysis_final_v2b.xls" --- you know the ones. It is a story about reimagining what's possible when we modernize how we collaborate as part of the open movement.

Openscapes has been co-developed by Julia Lowndes and Erin Robinson, combining their expertise of open data science and open data stewardship, community and program building, teaching and coaching, as well as their lived experience as researchers. Together, they built the Openscapes as a value-based initiative that builds momentum like a flywheel and sustainably supports research teams with open science. With the Openscapes Champions flagship program and the first cohort of research teams, Openscapes was initially launched by Julia through a Mozilla Fellowship in 2018.

Erin joined in 2020 and together they created the Openscapes Framework that supports scaling Openscapes into a movement with the Openscapes mentor community. Openscapes mentors are people embedded in science organizations that learn to lead Openscapes Champions programs for their organization. This framework was developed through a NASA grant that Julia and Erin lead. Beyond NASA, the Framework has supported mentors at several other organizations like NOAA, and the California Water Boards.

We are excited to continue learning and iterating with the open movement towards kinder, better science for future us.

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "A red sea urchin and a slightly smaller purple sea urchin sitting side-by-side"
knitr::include_graphics("images/minis/horst_openscapes_arctic_urchins.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_urchins.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR TEAM

Learn about our [**our team**](team.qmd) who are helping grow and improve Openscapes!

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "A brown-colored arctic crab facing forward."
knitr::include_graphics("images/minis/horst_openscapes_arctic_crab.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_crab.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR CODE OF CONDUCT

Openscapes is committed to providing a welcoming, safe, inclusive, and harassment-free experience for everyone.

We expect all participants to adhere to our [Code of Conduct](code_of_conduct.qmd), which was influenced and adapted from [rOpenSci](https://ropensci.org/code-of-conduct/) and [Mozilla](https://www.mozilla.org/en-US/about/governance/policies/participation/).

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "A blue-colored humpback whale swimming to the right."
knitr::include_graphics("images/minis/horst_openscapes_arctic_humpback.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_humpback.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR ART

[Allison Horst](https://allisonhorst.github.io/) creates original art for Openscapes. Formerly an artist in residence at NCEAS and Assistant Teaching Professor at the Bren School of Environmental Science & Management at the University of California, Santa Barbara, Allison now works as a Data Scientist Advocate at [Observable](https://observablehq.com/). She is also one of the inaugural Openscapes Champions. Allison has a background in the fine arts, and in recent years, began creating digital open artwork for data science and statistics education -- explore her [stats illustrations](https://allisonhorst.com/) and follow her on [Twitter](https://twitter.com/allison_horst).

See all of Allison's open landscapes on the [gallery page](gallery.qmd).

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "A blue-colored narwhal swimming to the left."
knitr::include_graphics("images/minis/horst_openscapes_arctic_narwhal.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_narwhal.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR LOGO

Our logo, designed by Allison Horst, represents both the open landscapes and the data within them.

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "17%"
#| fig-alt: "A stylized landscape showing rolling hills in different shades of orange, with purple flowers along the ridgelines. Subtle gridlines on the hills and circle packing in the flowers give a nod to data in the environment."
knitr::include_graphics("images/logo/openscapes_hex_design_final_correct_dimensions.png")
```
<!-- ![](images/logo/openscapes_hex_design_final_correct_dimensions.png){fig-align="center" width="200"} -->
:::

::: white-div
## OUR FUNDING

We thank our funders for supporting open science tools, people, and communities. Openscapes was originally created with support from [Mozilla](https://www.mozilla.org/), which also sponsored the [first Champions Cohort](events/2019-01-21-inaugural/index.Rmd). In 2019 we received additional support from the [Gordon and Betty Moore Foundation](https://www.moore.org/) to develop a strategic growth plan for Openscapes.

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "Three small puffins in a cluster facing left."
knitr::include_graphics("images/minis/horst_openscapes_arctic_puffins.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_puffins.png){fig-align="center" width="100"} -->
:::

::: white-div
## OUR WEBSITE

The Openscapes website was originally built by [Julia Lowndes](https://jules32.github.io/) in [`{blogdown}`](https://pkgs.rstudio.com/blogdown/) using the same open data science workflows that she uses for research and promotes through Openscapes. The website was rebuilt by [Samantha Csik](https://samanthacsik.github.io/) in spring 2023 using [Quarto](https://quarto.org/), an open-source scientific publishing tool used by many of the current Openscapes champions.

<br>

```{r}
#| echo: false
#| fig-align: center
#| out-width: "10%"
#| fig-alt: "A ship with an organge hull and silver instrumentation floats in blue water."
knitr::include_graphics("images/minis/horst_openscapes_arctic_ship.png")
```
<!-- ![](images/minis/horst_openscapes_arctic_ship.png){fig-align="center" width="100"} -->
:::
:::
